...
public class ServerSim implements ServerInterface
{
	...
	ArrayList<SuperRegionData> superRegionDataList;
	ArrayList<RegionData> regionDataList;
	
	public ServerSim(String pluginPath)
	{
		this.strongholdPath = pluginPath;
		recipeData = new RecipeData();
	}
	
	public void initServerData()
	{
		regionDataList = getRegionData(strongholdPath);
		superRegionDataList = getSuperRegionData(strongholdPath);
	}
	...
	private static ArrayList<RegionData> getRegionData(String path)
	{
...
//                System.out.println(e.getStackTrace());
	...
	public ArrayList<SuperRegionData> getSuperRegionList()
	{
		return superRegionDataList;
	}
	
	private static ArrayList<SuperRegionData> getSuperRegionData(String path)
	{
	...
	{
		for (SuperRegionData sRegion : superRegionDataList)
		{
			if (sRegion.getName().endsWith(superregionName))
			{
				String name  = sRegion.getName();
				Location l   = new Location(null, sRegion.getL().getX(), sRegion.getL().getY(), sRegion.getL().getZ());
				String type  = sRegion.getType();
				List<String> owner = sRegion.getOwners();
				Map<String, List<String>> members = sRegion.getMembers();
				int power = sRegion.getPower();
				double taxes = sRegion.getTaxes();
				double balance = sRegion.getBalance();
				LinkedList<Double> taxRevenue = sRegion.getTaxRevenue();
				int maxPower = sRegion.getMaxPower();
				
				return new SuperRegion(name, l, type, owner, members, power, taxes, balance, taxRevenue, maxPower); 
			}
		}
		return null;
	}
