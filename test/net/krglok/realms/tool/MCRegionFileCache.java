...
public class MCRegionFileCache {
 
    private static final int MAX_CACHE_SIZE = 256;
 
 
    private static final Map<File, Reference<MCRegionFile>> cache = new HashMap<File, Reference<MCRegionFile>>();
 
    private MCRegionFileCache() {
    }
 
    public static synchronized MCRegionFile getRegionFile(File basePath, int chunkX, int chunkZ) {
        File regionDir = new File(basePath, "region");
        File file = new File(regionDir, "r." + (chunkX >> 5) + "." + (chunkZ >> 5) + ".mcr");
 
        Reference<MCRegionFile> ref = cache.get(file);
        ...
        if (cache.size() >= MAX_CACHE_SIZE) {
            MCRegionFileCache.clear();
        }
 
        MCRegionFile reg = new MCRegionFile(file);
        cache.put(file, new SoftReference<MCRegionFile>(reg));
        return reg;
    }
 
    public static synchronized void clear() {
        for (Reference<MCRegionFile> ref : cache.values()) {
            try {
                if (ref.get() != null) {
                    ref.get().close();
                }
    ...
    public static int getSizeDelta(File basePath, int chunkX, int chunkZ) {
        MCRegionFile r = getRegionFile(basePath, chunkX, chunkZ);
